...
const support_settings = {
    ...
    "moderator": ["Мл. Модератор Discord", "Модератор Discord"], // Модераторы отвечающие на жалобы (по умолчанию)
    "administrators": ["❖ Администратор 3 ур. ❖", "❖ Администратор 4 ур. ❖"], // Дополнительные модераторы (администрация)
    "log_channel": "reports", // Канал для логирования действий
    "time_warning": 6000000, // Время напоминания активных жалоб (1 час - 6000000)
            ...
            if (answer.length == 1) {
                await channel.fetchMessage(answer[0].message).then(async support_message => {
                    ...
                    await connection.query(`SELECT * FROM \`tickets-new\` WHERE \`server\` = '${server.id}'`, (error, res) => {
                        ...
                        image.setImage("https://imgur.com/k3KSG6e");
                        support_message.edit('**Добро пожаловать, уважаемый пользователь Discord`a, ты попал в техническую поддержку\`Arizona Role Play Surprise**')
                    });
                }).catch(() => {
